...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = Mr(src1 + imm, 4));
	...
	//jalr
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, t = s->pc + 4, s->dnpc = (imm + src1) & ~1, R(dest) = t);
	...
	INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc   , I, t = R_CSR(imm), W_CSR(imm, t & (~src1)), R(dest) = t);
	...
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = ((uint64_t)((uint64_t)src1 * (uint64_t)src2))  >> 32);
	INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(dest) = ((long long)(SEXT(src1, 32) * (uint64_t)src2)) >> 32);
