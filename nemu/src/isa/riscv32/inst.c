...
#define immJ() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 19 | SEXT(BITS(i, 19, 12), 8) << 11 | SEXT(BITS(i, 20, 20), 1) << 10 | BITS(i, 30, 21)) << 1; } while(0)
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 11 | SEXT(BITS(i, 7, 7), 1) << 10 | SEXT(BITS(i, 30, 25), 6) << 4 | BITS(i, 11, 8)) << 1; } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	...
	INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, R(dest) = ((sword_t)src1 >> (imm & 0x0000003f)));
	INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = 0x000000ff & Mr(src1 + imm, 1));
	INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = src1 ^ imm);
	//要符号拓展(说着要拓展，但自己即没写拓展，，，)
	INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT(Mr(src1 + imm, 2), 16));
	//这个是0拓展
	INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = 0x0000ffff & Mr(src1 + imm, 2));
	INSTPAT("000000 ?????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> imm);


	//jump
	INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4, s->dnpc = 0x90000000);//
