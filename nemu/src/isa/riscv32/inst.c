...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	...
	INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, if(imm >> 5 == 0) R(dest) = src1 << imm);//judge
	INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, if(imm << 2 >> 7 == 0) R(dest) = ((sword_t)src1 >> (imm & 0x3f)));//<<26
	...
	//INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = ((0x00000000ffffffff & SEXT(src1, 32)) * (0x00000000ffffffff & SEXT(src2, 32))) >> 32);
//	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = ((uint64_t) src1 * (uint64_t) src2) >> 32);
	...
	//div有符号
	INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = SEXT(src1, 32) / SEXT(src2, 32));
	...
	INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh        , S, Mw(src1 + imm, 2, 0x0000ffff & src2));
	INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb        , S, Mw(src1 + imm, 1, 0x000000ff & src2));
