...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = Mr(src1 + imm, 4));
	...
	INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, (SEXT(src1, 32) < imm) ? (R(dest) = 1) : (R(dest) = 0));
	INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = imm & src1);
	INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << imm);//judge
	INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = ((sword_t)src1 >> (imm & 0x3f)));//<<26
	INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + imm, 1));
	INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = src1 ^ imm);
	//要符号拓展(说着要拓展，但自己即没写拓展，，，)
	INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT(Mr(src1 + imm, 2), 16));
	//这个是0拓展
	INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = 0xffff & Mr(src1 + imm, 2));
	INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, if(imm >> 5 == 0) R(dest) = src1 >> imm);//judge
	...
	INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs   , I, printf("reg = %s, val = %u\n", reg_name(dest, 8), R_CSR(imm)), R(dest) = R_CSR(imm), W_CSR(imm, R_CSR(imm) | src1));
	...
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = ((0xffffffff & SEXT(src1, 32)) * (0xffffffff & SEXT(src2, 32))) >> 32);
//	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = ((uint64_t) src1 * (uint64_t) src2) >> 32);
	...
	INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, ((sword_t)src1 < (sword_t)src2) ? (R(dest) = 1) : (R(dest) = 0));

	//div有符号,我也不知道这个有没有实现正确
	INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = (int)src1 / (int)src2); 
	INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(dest) = (SEXT(src1, 32) & 0xffffffff) / (SEXT(src2, 32) & 0xffffffff));
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	...
	INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = R_CSR(0x341) + 4);
	/*保存的是trap指令这一条的位置，需要及时+4*/
