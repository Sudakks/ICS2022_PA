...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	...
	//seqz伪命令，即imm = 1
	/*INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, (src1 < imm) ? (R(dest) = 1) : (R(dest) = 0));//TODO
	INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = imm & src1);
	INSTPAT("000000 ?????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << imm);//TODO
	...
	//要符号拓展
	INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT(Mr(src1 + imm, 2), 16));//TODO
	//这个是0拓展
...
/*	INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = (src1 * src2));
	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) = (int64_t)((int64_t)src1 * (int64_t)src2) >> 32);
	INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(dest) = src1 ^ src2);
	INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = src1 | src2);
	INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, (src1 < src2) ? (R(dest) = 1) : (R(dest) = 0));
	//只取src2的低5位
		INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = src1 << (0x0000001f & src2));//TODO
	INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2);
	INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, ((sword_t)src1 < (sword_t)src2) ? (R(dest) = 1) : (R(dest) = 0));
	//div有符号
	INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = (sword_t)src1 / (sword_t)src2);//TODO
...
*/
	...
	INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt      , B,
		if((sword_t)src1 < (word_t)src2)	s->dnpc = s->pc + imm
	);
