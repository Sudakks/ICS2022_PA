...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	...
	//这个是0拓展
	INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = Mr(src1 + imm, 2) & 0xffffffff);
	...
	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) = ((long long) ((long long)src1 * (long long)src2) >> 32) & 0x00000000ffffffff);
