...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	...
	INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, R(dest) = ((int)src1 >> (imm & 0x0000003f)));
...
//	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) = ((uint64_t)src1 * (uint64_t)src2) >> 32);
	...
	//只取src2的低5位
	INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = src1 << (0x0000001f & src2));
	INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2);
	INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, ((int)src1 < (int)src2) ? (R(dest) = 1) : (R(dest) = 0));
